...
TEWS Colour: ${result.tews_colour || 'N/A'}

Vitals:
- Heart Rate: ${vitals.heart_rate || 'N/A'} bpm
- Respiratory Rate: ${vitals.respiratory_rate || 'N/A'} /min
...
- AVPU: ${vitals.avpu || 'Not recorded'}

Clinical Discriminators:
${Object.keys(redFlags).filter(k => redFlags[k]).map(k => `• ${k.replace(/_/g, ' ')}`).join('\n') || '• None'}
...
-- HealthSync • Offline Triage Tool`;
    ...
    navigator.clipboard.writeText(summary)
        .then(() => alert("✅ Summary copied!\nPaste into LHIMS or doctor handover."))
